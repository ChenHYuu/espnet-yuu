...
class AbsTask(ABC):
    ...
    def get_parser(cls) -> config_argparse.ArgumentParser:
        ...
        group.add_argument(
            ...
            type=int,
            default=3,
            help="The number images to plot the stft outputs from frontend. "
            "This option makes sense only when attention-based model. "
            "We can also disable the spectrum plot by setting it 0",
        )
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if args.dry_run:
            ...
            if args.num_spectrum_plot != 0:
                plot_spectrum_iter_factory = cls.build_iter_factory(
                    args=args,
                    distributed_option=distributed_option,
                    mode="plot_spectrum"
                )
            else:
                plot_spectrum_iter_factory = None
            ...
            cls.trainer.run(
                ...
                distributed_option=distributed_option,
                plot_spectrum_iter_factory=plot_spectrum_iter_factory,
            )
    ...
    def build_iter_options(
        ...
        # (Su) add     
            ...
            batch_size = 1
            batch_bins = 0
            num_batches = args.num_spectrum_plot
            max_cache_fd = args.max_cache_fd
            allow_multi_rates = args.allow_multi_rates
            # num_att_plot should be a few sample ~ 3, so cache all data.
            max_cache_size = np.inf if args.max_cache_size != 0.0 else 0.0
